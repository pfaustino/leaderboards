...
/**
 * Union of every game's allowedOrigins — used when no game config is
 * available (e.g. CORS preflight requests carry no body or query).
 */
function allConfiguredOrigins() {
  const all = new Set();
  for (const cfg of Object.values(games)) {
    for (const origin of cfg.allowedOrigins ?? []) all.add(origin);
  }
  return all;
}
...
export function isAllowedOrigin(origin, gameConfig) {
  ...
  if (gameConfig) {
    return (gameConfig.allowedOrigins ?? []).includes(origin);
  }
  return allConfiguredOrigins().has(origin);
}
